...
class BinaryProtocol(Protocol):
    ...
    def deserialize(self, data: bytes) -> Message:
        try:
            ...
            offset += 1
            sender = data[offset : offset + sender_length].decode("utf-8") if sender_length > 0 else None
            offset += sender_length

            recipient_length = struct.unpack_from("!B", data, offset)[0]
            offset += 1
            recipient = data[offset : offset + recipient_length].decode("utf-8") if recipient_length > 0 else None
